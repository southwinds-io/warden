...
func Tap(uri, credential, address, token string, verbose, insecureskipverify bool) {
	...
	_ = os.Setenv(string(handler.TapDomain), uri)
	_ = os.Setenv(string(handler.TapInsecureSkipVerify), strconv.FormatBool(insecureskipverify))
	if len(credential) > 0 {
		creds := strings.Split(credential, ":")
		if len(creds) > 0 {
			_ = os.Setenv(string(handler.TapUserName), creds[0])
			_ = os.Setenv(string(handler.TapPassword), creds[1])
		} else {
	...
	if len(token) > 0 {
		_ = os.Setenv(string(handler.TapBearerToken), token)
	}
