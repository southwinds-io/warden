...
func (rd *RequestHandler) Handle(req *http.Request, _ *goproxy.ProxyCtx) {
	if req == nil {
		...
		uri, err := url.Parse(req.URL.String())
		if err != nil {
		...
		uri.Host = os.Getenv(string(TapDomain))
		log.Println(" new URL is ", uri.String())
		// build duplicate request using cloned body
		dupReq, err := http.NewRequest(req.Method, uri.String(), io.NopCloser(bytes.NewReader(body)))
		if err != nil {
